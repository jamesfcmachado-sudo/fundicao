...
def pagina_nova_of() -> None:
    ...
    # ── Campos na sequência de cadastro ──────────────────────────────────
    st.markdown("##### Dados da OF")
    st.caption("💡 Campos com **(▼ sugestões)** mostram valores já cadastrados. Você pode digitar qualquer valor livremente.")

    c1, c2 = st.columns(2)
    with c1:
        st.caption("**Número da OP:** formato `000A0` — 3 dígitos + letra A–L + 1 dígito.")
        numero_of   = st.text_input("Número da OP *", placeholder="001A6", key="_of_nof")
        numero_nn   = st.text_input("NN°", key="_of_nnn")
        nome_cliente = st.text_input(
            "Nome do Cliente * (▼ sugestões)",
            value=st.session_state.get("_sug_cliente", ""),
            key="_of_cliente",
        )
        st.selectbox("  ↳ selecionar cliente existente", options=[""] + _opts_cliente,
                     index=0, key="_sug_cliente",
                     help="Selecione para preencher o campo acima")
        data_abertura = st.date_input("Data de abertura do pedido *", value=date.today(),
                                      format=FORMATO_DATE_INPUT_BR, key="_of_data_ab")
        prazo_entrega = st.date_input("Prazo de entrega do pedido", value=None,
                                      format=FORMATO_DATE_INPUT_BR,
                                      min_value=date(1900,1,1), max_value=date(2100,12,31),
                                      key="_of_prazo")
        numero_pedido = st.text_input("Número do pedido", key="_of_nped")
        numero_modelo = st.text_input(
            "Número do modelo (▼ sugestões)",
            value=st.session_state.get("_sug_modelo", ""),
            key="_of_modelo",
        )
        st.selectbox("  ↳ selecionar modelo existente", options=[""] + _opts_modelo,
                     index=0, key="_sug_modelo")

    with c2:
        descricao_peca = st.text_area(
            "Descrição da peça (▼ sugestões)",
            value=st.session_state.get("_sug_descr", ""),
            key="_of_descr",
        )
        st.selectbox("  ↳ selecionar descrição existente", options=[""] + _opts_descr,
                     index=0, key="_sug_descr")
        numero_desenho = st.text_input(
            "Número do desenho (▼ sugestões)",
            value=st.session_state.get("_sug_desenho", ""),
            key="_of_desenho",
        )
        st.selectbox("  ↳ selecionar desenho existente", options=[""] + _opts_desenho,
                     index=0, key="_sug_desenho")
        peso_liquido = st.number_input("Peso líquido (kg)", min_value=0.0, value=0.0, format="%.4f", key="_of_pl")
        peso_bruto   = st.number_input("Peso bruto (kg)",   min_value=0.0, value=0.0, format="%.4f", key="_of_pb")
        liga  = st.text_input(
            "Liga (▼ sugestões)",
            value=st.session_state.get("_sug_liga", ""),
            key="_of_liga",
        )
        st.selectbox("  ↳ selecionar liga existente", options=[""] + _opts_liga, index=0, key="_sug_liga")
        norma = st.text_input(
            "Norma (▼ sugestões)",
            value=st.session_state.get("_sug_norma", ""),
            key="_of_norma",
        )
        st.selectbox("  ↳ selecionar norma existente", options=[""] + _opts_norma, index=0, key="_sug_norma")
        qtd_pedido  = st.number_input("Quantidade de peças do pedido *", min_value=0, value=0, step=1, key="_of_qtdp")
        qtd_fundida = st.number_input("Qtde fundida",  min_value=0, value=0, step=1, key="_of_qtdf")
        qtd_expedida = st.number_input("Qtde expedida", min_value=0, value=0, step=1, key="_of_qtde")

    valor_unit  = st.number_input("Valor unitário", min_value=0.0, value=0.0, format="%.6f", key="_of_vunit")
    # Valor total calculado automaticamente: qtd_pedido × valor_unitário
    _vtot_calc  = float(st.session_state.get("_of_qtdp", 0) or 0) * float(st.session_state.get("_of_vunit", 0.0) or 0.0)
    valor_total = st.number_input("Valor total (preenchido automaticamente)", min_value=0.0,
                                  value=_vtot_calc, format="%.2f", key="_of_vtot")
    condicao_modelo = st.text_input("Condição do modelo", key="_of_cond")
    observacoes     = st.text_area("Observações gerais da OF", key="_of_obs")

    st.divider()
    st.markdown("##### Ordens de entrega (OE) e certificados")
    st.caption("Defina quantas linhas deseja preencher.")
    ...
    if n_certs > 0:
        ...
        for i in range(int(n_certs)):
            ...
            with _cb:
                st.number_input(f"Certificado {i+1} — qtd peças", min_value=0, value=0, step=1, key=f"ce_q_{i}")

    st.divider()
    enviar = st.button("💾 Salvar no banco de dados", type="primary", key="btn_salvar_of")

    if not enviar:
        return

    # ── Lê valores dos widgets via session_state ──────────────────────────
    numero_of      = (st.session_state.get("_of_nof")      or "").strip()
    numero_nn      = (st.session_state.get("_of_nnn")      or "").strip() or None
    # Para campos com sugestão: usa o text_input (digitado) se preenchido,
    # caso contrário usa o selectbox
    nome_cliente   = (st.session_state.get("_of_cliente")  or st.session_state.get("_sug_cliente") or "").strip()
    data_abertura  = st.session_state.get("_of_data_ab",  date.today())
    prazo_entrega  = st.session_state.get("_of_prazo",    None)
    numero_pedido  = (st.session_state.get("_of_nped")    or "").strip() or None
    numero_modelo  = (st.session_state.get("_of_modelo")  or st.session_state.get("_sug_modelo")  or "").strip() or None
    descricao_peca = (st.session_state.get("_of_descr")   or st.session_state.get("_sug_descr")   or "").strip() or None
    numero_desenho = (st.session_state.get("_of_desenho") or st.session_state.get("_sug_desenho") or "").strip() or None
    peso_liquido   = float(st.session_state.get("_of_pl",  0.0) or 0.0)
    peso_bruto     = float(st.session_state.get("_of_pb",  0.0) or 0.0)
    liga           = (st.session_state.get("_of_liga")    or st.session_state.get("_sug_liga")    or "").strip() or None
    norma          = (st.session_state.get("_of_norma")   or st.session_state.get("_sug_norma")   or "").strip() or None
    qtd_pedido     = int(st.session_state.get("_of_qtdp",  0) or 0)
    qtd_fundida    = int(st.session_state.get("_of_qtdf",  0) or 0)
    qtd_expedida   = int(st.session_state.get("_of_qtde",  0) or 0)
    valor_unit_v   = float(st.session_state.get("_of_vunit", 0.0) or 0.0)
    valor_total_v  = float(qtd_pedido * valor_unit_v) if valor_unit_v else float(st.session_state.get("_of_vtot", 0.0) or 0.0)
    condicao_modelo = (st.session_state.get("_of_cond")  or "").strip() or None
    observacoes    = (st.session_state.get("_of_obs")    or "").strip() or None

    if not numero_of or not nome_cliente:
    ...
    of = OrdemFabricacao(
        ...
        liga=liga,
        norma=norma,
        qtd_pecas_pedido=qtd_pedido,
        qtd_fundida=qtd_fundida,
        qtd_expedida=qtd_expedida,
        valor_unitario=Decimal(str(valor_unit_v)) if valor_unit_v else None,
        valor_total=Decimal(str(valor_total_v)) if valor_total_v else None,
                                ...
                                with st.form(f"form_alt_of_{_nof_sel}", clear_on_submit=False):
                                    ...
                                    if st.form_submit_button("💾 Salvar alterações", type="primary"):
                                        try:
                                            ...
                                            st.success(f"OF **{_nof_sel}** atualizada com sucesso!")
