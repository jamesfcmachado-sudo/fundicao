...
def pagina_nova_of() -> None:
    ...
    # ── Campos na sequência de cadastro — selectboxes integrados ─────────
    st.markdown("##### Dados da OF")
    st.caption("💡 Campos marcados com **▼** oferecem sugestões de valores já cadastrados ao digitar.")

    c1, c2 = st.columns(2)
    with c1:
        st.caption("**Número da OP:** formato `000A0` — 3 dígitos + letra A–L + 1 dígito.")
        _nof   = st.text_input("Número da OP *", placeholder="001A6",
                               help="Regex: ^\\d{3}[A-L]\\d$ — apenas maiúsculas na letra central.",
                               key="_of_nof")
        _nnn   = st.text_input("NN°", key="_of_nnn")
        st.selectbox("Nome do Cliente * ▼", options=[""] + _opts_cliente, index=0, key="_of_cliente")
        st.date_input("Data de abertura do pedido *", value=date.today(),
                      format=FORMATO_DATE_INPUT_BR, key="_of_data_ab")
        st.date_input("Prazo de entrega do pedido", value=None,
                      format=FORMATO_DATE_INPUT_BR,
                      min_value=date(1900, 1, 1), max_value=date(2100, 12, 31),
                      key="_of_prazo")
        _nped  = st.text_input("Número do pedido", key="_of_nped")
        st.selectbox("Número do modelo ▼", options=[""] + _opts_modelo, index=0, key="_of_modelo")
    with c2:
        st.selectbox("Descrição da peça ▼", options=[""] + _opts_descr, index=0, key="_of_descr")
        st.selectbox("Número do desenho ▼", options=[""] + _opts_desenho, index=0, key="_of_desenho")
        _pl    = st.number_input("Peso líquido (kg)", min_value=0.0, value=0.0, format="%.4f", key="_of_pl")
        _pb    = st.number_input("Peso bruto (kg)",   min_value=0.0, value=0.0, format="%.4f", key="_of_pb")
        st.selectbox("Liga ▼",  options=[""] + _opts_liga,  index=0, key="_of_liga")
        st.selectbox("Norma ▼", options=[""] + _opts_norma, index=0, key="_of_norma")
        _qtdp  = st.number_input("Quantidade de peças do pedido *", min_value=0, value=0, step=1, key="_of_qtdp")
        _qtdf  = st.number_input("Qtde fundida",  min_value=0, value=0, step=1, key="_of_qtdf")
        _qtde  = st.number_input("Qtde expedida", min_value=0, value=0, step=1, key="_of_qtde")

    _vunit = st.number_input("Valor unitário", min_value=0.0, value=0.0, format="%.6f", key="_of_vunit")
    _vtot  = st.number_input("Valor total",    min_value=0.0, value=0.0, format="%.2f",  key="_of_vtot")
    _cond  = st.text_input("Condição do modelo", key="_of_cond")
    _obs   = st.text_area("Observações gerais da OF", key="_of_obs")
    ...
    numero_of      = st.session_state.get("_of_nof", "")
    numero_nn      = st.session_state.get("_of_nnn", "")
    nome_cliente   = st.session_state.get("_of_cliente", "")
    data_abertura  = st.session_state.get("_of_data_ab", date.today())
    prazo_entrega  = st.session_state.get("_of_prazo", None)
    numero_pedido  = st.session_state.get("_of_nped", "")
    numero_modelo  = st.session_state.get("_of_modelo", "")
    descricao_peca = st.session_state.get("_of_descr", "")
    numero_desenho = st.session_state.get("_of_desenho", "")
    peso_liquido   = st.session_state.get("_of_pl", 0.0)
    peso_bruto     = st.session_state.get("_of_pb", 0.0)
    liga           = st.session_state.get("_of_liga", "")
    norma          = st.session_state.get("_of_norma", "")
    qtd_pedido     = st.session_state.get("_of_qtdp", 0)
    qtd_fundida    = st.session_state.get("_of_qtdf", 0)
    qtd_expedida   = st.session_state.get("_of_qtde", 0)
    valor_unit     = st.session_state.get("_of_vunit", 0.0)
    valor_total    = st.session_state.get("_of_vtot",  0.0)
    condicao_modelo = st.session_state.get("_of_cond", "")
    observacoes    = st.session_state.get("_of_obs",  "")
