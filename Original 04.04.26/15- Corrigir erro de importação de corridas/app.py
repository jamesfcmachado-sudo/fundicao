...
def pagina_relatorios() -> None:
    ...
    with tab4:
        ...
        st.caption(
            "Se a importação de corridas estiver retornando erros de duplicidade, "
            "clique abaixo para remover o índice único da tabela de corridas. "
            "Esta operação é segura e não apaga dados."
        )
        if st.button("🔧 Remover índice único de corridas", key="btn_drop_idx"):
            try:
                import sqlalchemy as _sa
                _engine = SessionLocal().bind  # type: ignore[attr-defined]
                with _engine.connect() as _conn:
                    # Busca TODOS os índices unique da tabela corrida
                    _rows = _conn.execute(_sa.text(
                        "SELECT name FROM sqlite_master "
                        "WHERE type='index' AND tbl_name='corrida'"
                    )).fetchall()
                    _dropped = []
                    for (_name,) in _rows:
                        # Pega o SQL de criação para checar se é UNIQUE
                        _sql_row = _conn.execute(_sa.text(
                            f"SELECT sql FROM sqlite_master WHERE name='{_name}'"
                        )).fetchone()
                        _sql = (_sql_row[0] or "") if _sql_row else ""
                        if "UNIQUE" in _sql.upper():
                            _conn.execute(_sa.text(f'DROP INDEX IF EXISTS "{_name}"'))
                            _dropped.append(_name)
                    _conn.commit()
                if _dropped:
                    st.success(f"✅ Índice(s) removido(s): {', '.join(_dropped)}. Tente importar novamente.")
                else:
                    st.info("Nenhum índice único encontrado na tabela corrida. O banco já está correto.")
            except Exception as _exc:
                st.error(f"Erro ao remover índice: {_exc}")
...
def _importar_corridas(arquivo) -> None:
    ...
    # Garante remoção do índice único ANTES de inserir — cobre casos onde
    # a migração automática falhou (banco criado com versão anterior do código)
    try:
        import sqlalchemy as _sa_imp
        _eng = SessionLocal().bind  # type: ignore[attr-defined]
        with _eng.connect() as _c_imp:
            _idx_rows = _c_imp.execute(_sa_imp.text(
                "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='corrida'"
            )).fetchall()
            for (_iname,) in _idx_rows:
                _isql_r = _c_imp.execute(_sa_imp.text(
                    f"SELECT sql FROM sqlite_master WHERE name='{_iname}'"
                )).fetchone()
                _isql = (_isql_r[0] or '') if _isql_r else ''
                if 'UNIQUE' in _isql.upper():
                    _c_imp.execute(_sa_imp.text(f'DROP INDEX IF EXISTS "{_iname}"'))
            _c_imp.commit()
    except Exception:
        pass  # Silencioso — prossegue para inserção
    ...
    for _, row in df.iterrows():
        try:
            ...
            # Só ocorre se o banco ainda tiver índice único antigo.
            # Nesse caso registra e continua — não trava as demais linhas.
            erros.append(
                f"Linha {_ + 2} — conflito no banco para corrida \"{row.get('numero_corrida', '?')}\""
                f" / OF \"{nof or '—'}\" / Série \"{serie or '—'}\"."
                f" Execute a migração: DROP INDEX uq_corridas_numero_data_of"
            )
        except Exception as exc:
            erros.append(f"Linha {_ + 2}: {exc}")
