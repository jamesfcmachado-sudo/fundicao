...
def _chave_of(codigo: str, data_abertura: str = "") -> tuple:
    """
    Chave de ordenação para códigos no formato 000X0 (ex: 001A6, 002K9).

    Regras:
    1. Remove sufixo "dev" (ex: "001A6dev" → trata como "001A6").
    2. Extrai: número sequencial, letra, dígito final.
    3. Ordena por: letra → número sequencial → dígito final.
       Assim: 001A6, 002A6... 001B6... 001K9, 002K9...
    4. A data de abertura entra como desempate final (mais antiga primeiro).
    5. Códigos fora do padrão vão ao final em ordem alfabética.
    """
    import re as _re
    if not codigo or not isinstance(codigo, str):
        return ("ZZ", 9999, 99, "9999-99-99", codigo or "")
    # Remove "dev" (case-insensitive) de qualquer posição
    limpo = _re.sub(r"(?i)dev", "", codigo.strip()).strip()
    m = _re.fullmatch(r"(\d{1,4})([A-Za-z])(\d)", limpo)
    if m:
        num, letra, sfx = m.groups()
        return (letra.upper(), int(num), int(sfx), data_abertura or "9999-99-99", codigo)
    return ("ZZ", 9999, 99, data_abertura or "9999-99-99", codigo)
...
def pagina_relatorios() -> None:
    ...
    with tab1:
        if not ofs:
            ...
            # Ordena pela chave alfanumérica do número da OF
            # "dev" é ignorado; data de abertura desempata OFs com padrões diferentes
            if "Nº OF" in df.columns:
                _chaves = [
                    _chave_of(of, data)
                    for of, data in zip(
                        df["Nº OF"].fillna(""),
                        df["Abertura"].fillna("") if "Abertura" in df.columns else [""] * len(df)
                    )
                ]
                df = df.iloc[pd.Index(_chaves).argsort()].reset_index(drop=True)
            ...
            _OF_COL_CFG = {
                ...
                "Nº OE":           st.column_config.TextColumn("Nº OE",            width="small"),
                "Nº Certificado":  st.column_config.TextColumn("Nº Certificado",   width="small"),
            }
    ...
    with tab2:
        if not corridas:
            ...
            # Ordena pela chave alfanumérica do número da corrida
            if "Corrida" in df.columns:
                _chaves_c = [
                    _chave_of(corr, data)
                    for corr, data in zip(
                        df["Corrida"].fillna(""),
                        df["Data fusão"].fillna("") if "Data fusão" in df.columns else [""] * len(df)
                    )
                ]
                df = df.iloc[pd.Index(_chaves_c).argsort()].reset_index(drop=True)
