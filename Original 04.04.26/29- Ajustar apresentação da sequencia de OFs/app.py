...
def pagina_relatorios() -> None:
    ...
    with tab1:
        if not ofs:
            ...
            # Ordena: 1º por data de abertura, 2º pela chave alfanumérica da OF
            # "dev" é ignorado na chave. Resultado: mais antigas primeiro,
            # dentro do mesmo dia segue 001A6, 002A6... 001B6... 001K9...
            if "Nº OF" in df.columns and "Abertura" in df.columns:
                ...
                df = df.sort_values(
                    by=["_sort_data", "_sort_of"],
                    ascending=[True, True],
                    na_position="last",
                ).drop(columns=["_sort_data", "_sort_of"]).reset_index(drop=True)
    ...
    with tab2:
        if not corridas:
            ...
            if "Corrida" in df.columns:
                df = df.iloc[
                    pd.Index(df["Corrida"].fillna("").apply(_chave_of)).argsort()
