...
def _montar_linhas_of(ofs_list) -> list[dict]:
    ...
    OEs e certificados são achatados: cada OF gera uma linha por OE+cert,
    ou uma única linha se não houver nenhum.
    Sequência: numero_of, numero_nn, nome_cliente, data_abertura_pedido,
    prazo_entrega_pedido, numero_pedido, numero_modelo, descricao_peca,
    numero_desenho, peso_liquido_kg, peso_bruto_kg, liga, norma,
    qtd_pecas_pedido, qtd_fundida, qtd_expedida, valor_unitario, valor_total,
    Cond. Modelo, observacoes, numero_oe, numero_certificado.
    """
    rows = []
    for of in ofs_list:
        base = {
            ...
            "Cond. Modelo":         of.condicao_modelo or "",
            "observacoes":          of.observacoes or "",
        }

        oes   = list(getattr(of, "ordens_entrega",  []) or [])
        certs = list(getattr(of, "certificados",    []) or [])
...
def pagina_dashboard():
    ...
    # Carrega OEs e certificados junto com as OFs para exibição completa
    try:
        with db_session() as db:
            ...
            todas_completas = list(
                db.scalars(
                    ...
                    .options(_sl(OrdemFabricacao.ordens_entrega), _sl(OrdemFabricacao.certificados))
                ).all()
            )
    except Exception:
        todas_completas = todas  # fallback sem OE/cert
    ...
    abertas_ct   = sum(1 for of in abertas_list if _status_of(of) == "Aberta")
    parciais_ct  = sum(1 for of in abertas_list if _status_of(of) == "Expedição parcial")

    m2.metric("OFs em acompanhamento", len(abertas_list))
    ...
    elif not abertas_list:
        st.info("Todas as OFs estão encerradas.")
    else:
        linhas = _montar_linhas_of(abertas_list)
        st.dataframe(
            pd.DataFrame(linhas),
            height=400,
...
def pagina_relatorios() -> None:
    ...
    with tab1:
        if not ofs:
            ...
            # Recarrega com OEs e certificados para exibição completa
            try:
                with db_session() as db:
                    from sqlalchemy.orm import selectinload as _sl
                    ofs_completas = list(
