...
def _importar_corridas(arquivo) -> None:
    ...
    # ── Dropa TODOS os índices únicos de corridas via sqlite3 puro ────
    # sqlite3 não depende do SQLAlchemy e sempre funciona com fundicao.db
    try:
        with sqlite3.connect('fundicao.db') as _con_idx:
            _idx = _con_idx.execute(
                "SELECT name, sql FROM sqlite_master "
                "WHERE type='index' AND tbl_name='corrida' AND sql IS NOT NULL"
            ).fetchall()
            for _ni, _si in _idx:
                if _si and 'UNIQUE' in _si.upper():
                    _con_idx.execute(f'DROP INDEX IF EXISTS "{_ni}"')
            _con_idx.commit()
    except Exception:
