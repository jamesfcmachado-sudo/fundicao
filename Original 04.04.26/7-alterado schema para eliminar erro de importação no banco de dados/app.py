    ...
    for _, row in df.iterrows():
        try:
            ...
            of = OrdemFabricacao(
                ...
                peso_liquido_kg=Decimal(str(_val("peso_liquido_kg", 0) or 0)) or None,
                peso_bruto_kg=Decimal(str(_val("peso_bruto_kg", 0) or 0)) or None,
                liga=str(_val("liga", "") or "").strip() or None,
                norma=str(_val("norma", "") or "").strip() or None,
                qtd_pecas_pedido=int(_val("qtd_pecas_pedido", 0) or 0),
                qtd_fundida=int(_val("qtd_fundida", 0) or 0),
                qtd_expedida=int(_val("qtd_expedida", 0) or 0),
                valor_unitario=Decimal(str(_val("valor_unitario", 0) or 0)) or None,
                valor_total=Decimal(str(_val("valor_total", 0) or 0)) or None,
    ...
    for _, row in df.iterrows():
        try:
            ...
            corrida = Corrida(
                ...
                qtd_pecas_fundidas=int(_val("qtd_pecas_fundidas", 0) or 0),
                serie_pecas_fundidas=str(_val("serie_pecas_fundidas", "") or "").strip() or None,
                liga=str(_val("liga", "") or "").strip() or None,
                norma=str(_val("norma", "") or "").strip() or None,
                composicao_quimica_pct=composicao,
                criado_em=now,
            ...
            with db_session() as db:
                # merge() não lança IntegrityError se a PK já existir — insere ou atualiza.
                # Permite que o mesmo numero_corrida apareça com OFs distintas (sem unique isolado).
                db.merge(corrida)
            inseridos += 1
        except IntegrityError as ie:
            # Registra a linha problemática sem travar o restante da importação
            erros.append(
                f"Linha {_ + 2} — conflito de chave: corrida \"{row.get('numero_corrida', '?')}\""
                f" / OF \"{row.get('numero_ordem_fabricacao', '—')}\" já existe."
            )
        except Exception as exc:
            erros.append(f"Linha {_ + 2}: {exc}")
...
def _migrar_banco_corridas() -> None:
    ...
    Situação correta:   UNIQUE (numero_corrida, data_fusao, numero_ordem_fabricacao)
        ...
        if dialect == "sqlite":
            ...
            if old_index in index_names and new_index not in index_names:
                ...
                conn.execute(sqlalchemy.text(
                    ...
                    "ON corrida (numero_corrida, data_fusao, numero_ordem_fabricacao)"
                ))
                conn.commit()
            ...
            if row:
                ...
                conn.execute(sqlalchemy.text(
                    "CREATE UNIQUE INDEX uq_corridas_numero_data_of "
                    "ON corridas.corrida (numero_corrida, data_fusao, numero_ordem_fabricacao)"
                ))
                conn.commit()
