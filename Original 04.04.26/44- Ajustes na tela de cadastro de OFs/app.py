...
def pagina_nova_of() -> None:
    ...
    # Suprime mensagem de autopreenchimento do browser nos campos de texto
    st.markdown(
        """<style>
    ...
    st.markdown("##### Autocomplete — selecione valores já cadastrados")
    st.caption("Selecione um valor para pré-preencher o campo correspondente no formulário abaixo. Você pode editar livremente depois.")
    _ac1, _ac2 = st.columns(2)
    with _ac1:
        st.selectbox("Cliente", options=[""] + _opts_cliente, index=0, key="ac_cliente")
        st.selectbox("Número do modelo", options=[""] + _opts_modelo, index=0, key="ac_modelo")
        st.selectbox("Descrição da peça", options=[""] + _opts_descr, index=0, key="ac_descr")
    with _ac2:
        st.selectbox("Número do desenho", options=[""] + _opts_desenho, index=0, key="ac_desenho")
        st.selectbox("Liga", options=[""] + _opts_liga, index=0, key="ac_liga")
        st.selectbox("Norma", options=[""] + _opts_norma, index=0, key="ac_norma")
    st.divider()
    ...
    # Prazo de entrega fora do form para não sumir com clear_on_submit
    tem_prazo = st.checkbox("Informar prazo de entrega do pedido", value=False, key="chk_prazo_of")
    if tem_prazo:
        st.date_input(
            "Prazo de entrega do pedido",
            value=date.today(),
            format=FORMATO_DATE_INPUT_BR,
            key="prazo_entrega_pre",
    ...
    with st.form("form_nova_of", clear_on_submit=True):
        ...
        with c1:
            ...
            nome_cliente = st.text_input("Nome do Cliente *", value=st.session_state.get("ac_cliente", ""))
            data_abertura = st.date_input(
                "Data de abertura do pedido *",
                value=date.today(),
                format=FORMATO_DATE_INPUT_BR,
            )
            # Prazo definido fora do form (checkbox acima)
            if st.session_state.get("chk_prazo_of") and st.session_state.get("prazo_entrega_pre"):
                st.info(f"Prazo de entrega: **{st.session_state['prazo_entrega_pre'].strftime('%d/%m/%Y')}**")
            numero_pedido = st.text_input("Número do pedido")
            numero_modelo = st.text_input("Número do modelo", value=st.session_state.get("ac_modelo", ""))
        with c2:
            descricao_peca = st.text_area("Descrição da peça", value=st.session_state.get("ac_descr", ""))
            numero_desenho = st.text_input("Número do desenho", value=st.session_state.get("ac_desenho", ""))
            ...
            liga = st.text_input("Liga", value=st.session_state.get("ac_liga", ""))
            norma = st.text_input("Norma", value=st.session_state.get("ac_norma", ""))
    ...
    of = OrdemFabricacao(
        ...
        prazo_entrega_pedido=st.session_state.get("prazo_entrega_pre") if st.session_state.get("chk_prazo_of") else None,
