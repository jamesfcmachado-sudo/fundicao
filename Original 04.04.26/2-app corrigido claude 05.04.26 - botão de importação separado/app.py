...
def _importar_ofs(arquivo) -> None:
    ...
    st.dataframe(formatar_datas_br(df.head()), use_container_width=True, hide_index=True)
...
def _importar_corridas(arquivo) -> None:
    ...
    for _, row in df.iterrows():
        try:
            ...
            erros.append(f"Corrida duplicada: {row.get('numero_corrida', '?')} / {row.get('data_fusao', '?')}")
        except Exception as exc:
            erros.append(f"Linha {_ + 2}: {exc}")
