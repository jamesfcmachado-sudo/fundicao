...
def pagina_relatorios() -> None:
    ...
    with tab1:
        if not ofs:
            ...
            # Guarda o id de cada OF na mesma ordem do df para identificar a linha selecionada
            _ids_of = [r["numero_of"] for r in rows]
            ...
            # Identifica linha selecionada
            _idx_of = (sel_of.selection.rows or [None])[0]
            if _idx_of is not None:
                _nof_sel = _ids_of[_idx_of] if _idx_of < len(_ids_of) else None
                if _nof_sel:
                    st.divider()
                        ...
                        with st.expander("✏️ Alterar dados desta OF", expanded=False):
                            ...
                            if _of_data:
                                with st.form(f"form_alt_of_{_nof_sel}", clear_on_submit=False):
                                    ...
                                    with _c2:
                                        _prazo    = st.date_input("Prazo entrega", value=_of_data["prazo_entrega_pedido"], format="DD/MM/YYYY") if _of_data["prazo_entrega_pedido"] else st.date_input("Prazo entrega", value=None, format="DD/MM/YYYY")
                    ...
                    with st.expander("✏️ Alterar dados desta corrida", expanded=False):
                        ...
                        if _cd:
                            with st.form(f"form_alt_corr_{_id_corr_sel}", clear_on_submit=False):
                                ...
                                with _cc1a:
                                    _c_data   = st.date_input("Data fusão *", value=_cd["data_fusao"], format="DD/MM/YYYY")
