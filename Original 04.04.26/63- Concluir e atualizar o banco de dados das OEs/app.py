...
def pagina_consulta_oes():
    ...
    # ── Carregar dados do banco via SQL direto ──────────────────────────────
    try:
        import sqlite3 as _sq
        from fundicao_db import DB_PATH as _FP
    ...
    if sel:
        ...
        num_of_sel = str(oe_row["numero_of"])
        ...
        with st.expander(f"📋 Detalhes da OE {num_oe_sel} — OF {num_of_sel}", expanded=True):
            ...
            col_cli = "nome_cliente" if "nome_cliente" in oe_row.index and oe_row.get("nome_cliente") else "of_cliente"
            d3.text_input("Cliente", str(oe_row.get(col_cli, "")), disabled=True)
            d4.text_input("Qtde Total", str(int(oe_row.get("qtd_pecas", 0))), disabled=True)
            ...
            obs_val = str(oe_row.get("observacao", "") or "")
            if obs_val:
                st.text_area("Observação", obs_val, disabled=True, height=60)

            # Gerar PDF da OE selecionada
            st.divider()
