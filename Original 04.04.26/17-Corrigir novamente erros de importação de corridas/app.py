...
def _importar_corridas(arquivo) -> None:
    ...
    # ── Dropa índice único de corridas antes de inserir ─────────────────
    # Usa get_bind() via db_session — compatível com SQLAlchemy 1.x e 2.x.
    try:
        with db_session() as _db_idx:
            from sqlalchemy import text as _sqlt
            _engine_idx = _db_idx.get_bind()
            with _engine_idx.connect() as _cx_idx:
                _idx_rows = _cx_idx.execute(_sqlt(
                    "SELECT name, sql FROM sqlite_master "
                    "WHERE type='index' AND tbl_name='corrida' AND sql IS NOT NULL"
                )).fetchall()
                for _ni, _si in _idx_rows:
                    if _si and 'UNIQUE' in _si.upper():
                        _cx_idx.execute(_sqlt(f'DROP INDEX IF EXISTS "{_ni}"'))
                try:
                    _cx_idx.commit()
                except Exception:
                    pass
    except Exception:
        pass  # silencioso — não impede a importação
