...
def _migrar_banco_corridas() -> None:
    ...
    with engine.connect() as conn:
        if dialect == "sqlite":
            ...
            # Remove qualquer índice único de corridas — a regra de negócio
            # permite repetição de corrida+OF+série em combinações distintas.
            # A unicidade real é a PK (UUID), não uma combinação de campos.
            for _idx_drop in [old_index, new_index, "uq_corridas_numero_data_of"]:
                if _idx_drop in index_names:
                    conn.execute(sqlalchemy.text(f"DROP INDEX IF EXISTS {_idx_drop}"))
            conn.commit()
            ...
            # Remove todos os índices únicos de corridas no PostgreSQL
            for _idx in ["uq_corridas_numero_data", "uq_corridas_numero_data_of"]:
                conn.execute(sqlalchemy.text(
                    f"DROP INDEX IF EXISTS corridas.{_idx}"
                ))
            conn.commit()
