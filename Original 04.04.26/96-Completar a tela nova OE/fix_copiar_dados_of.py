...
OLD = '''                            # Se OF mudou, busca dados da nova OF
...
NEW = '''                            # Se OF mudou, busca e copia TODOS os dados da nova OF
                            if _of_edit != _of_atual_item and _of_edit in _ofs_lista:
                                _of_novo = _conn_upd.execute(_text_upd("""
                                    SELECT nome_cliente, numero_pedido, liga,
                                           descricao_peca, numero_modelo,
...
if OLD in src:
    ...
    print("OK: Copia todos os dados da nova OF.")
else:
...
try:
    ...
    print("SINTAXE OK! Rode: git add . && git commit -m 'Copia dados OF ao alterar item OE' && git push")
