...
def _montar_linhas_of(ofs_list) -> list[dict]:
    ...
    """
    # ── Carrega status_of via SQL direto — contorna cache do SQLAlchemy ──
    _status_map: dict[str, str] = {}
    try:
        import sqlite3 as _sq
        from fundicao_db import DB_PATH as _FP
        _cx = _sq.connect(str(_FP))
        _colnames = [r[1] for r in _cx.execute("PRAGMA table_info(ordem_fabricacao)").fetchall()]
        if "status_of" in _colnames:
            for _nr, _st in _cx.execute("SELECT numero_of, status_of FROM ordem_fabricacao"):
                _status_map[_nr] = _st or "Ativa"
        _cx.close()
    except Exception:
        pass
