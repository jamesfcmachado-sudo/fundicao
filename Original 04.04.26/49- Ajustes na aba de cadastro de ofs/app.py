...
def pagina_nova_of() -> None:
    ...
    # ── Carrega opções de sugestão do banco ───────────────────────────────
    try:
    ...
    def _campo_com_sugestao(label, key_input, opcoes, col_width=None):
        """
        Campo de texto livre com sugestões clicáveis abaixo.
        - Digitar qualquer valor: aceito normalmente.
        - Clicar em uma sugestão filtrada: preenche o campo via session_state.
        """
        # Se uma sugestão foi clicada, pré-carrega no input
        _preencher = st.session_state.pop(f"_fill_{key_input}", None)
        _valor_atual = st.session_state.get(key_input, "") or ""
        if _preencher:
            _valor_atual = _preencher

        val = st.text_input(label, value=_valor_atual, key=key_input)

        # Filtra sugestões pelo que foi digitado
        _termo = (_valor_atual or "").strip().lower()
        if _termo and opcoes:
            _sugestoes = [o for o in opcoes if _termo in o.lower()][:8]
            if _sugestoes:
                st.caption("Sugestões — clique para selecionar:")
                _cols = st.columns(min(len(_sugestoes), 4))
                for _i, _sug in enumerate(_sugestoes):
                    with _cols[_i % 4]:
                        if st.button(
                            _sug[:30] + ("…" if len(_sug) > 30 else ""),
                            key=f"_sug_{key_input}_{_i}",
                            help=_sug,
                            use_container_width=True,
                        ):
                            st.session_state[f"_fill_{key_input}"] = _sug
                            st.rerun()
        return val

    # ── Campos na sequência de cadastro ──────────────────────────────────
    st.markdown("##### Dados da OF")
    st.caption("💡 Ao digitar nos campos marcados com **(▼)**, aparecem sugestões clicáveis dos valores já cadastrados.")
    ...
    with c1:
        ...
        nome_cliente  = _campo_com_sugestao("Nome do Cliente * (▼)", "_of_cliente", _opts_cliente)
        ...
        numero_modelo = _campo_com_sugestao("Número do modelo (▼)", "_of_modelo", _opts_modelo)

    with c2:
        descricao_peca = _campo_com_sugestao("Descrição da peça (▼)", "_of_descr",   _opts_descr)
        numero_desenho = _campo_com_sugestao("Número do desenho (▼)", "_of_desenho", _opts_desenho)
        ...
        liga           = _campo_com_sugestao("Liga (▼)",  "_of_liga",  _opts_liga)
        norma          = _campo_com_sugestao("Norma (▼)", "_of_norma", _opts_norma)
    ...
    _qtdp_now   = int(st.session_state.get("_of_qtdp", 0) or 0)
    _vunit_now  = float(st.session_state.get("_of_vunit", 0.0) or 0.0)
    _vtot_calc  = _qtdp_now * _vunit_now
    valor_total = st.number_input("Valor total (qtd × vlr unitário)", min_value=0.0,
    ...
    numero_modelo  = (st.session_state.get("_of_modelo")  or "").strip() or None
    descricao_peca = (st.session_state.get("_of_descr")   or "").strip() or None
    numero_desenho = (st.session_state.get("_of_desenho") or "").strip() or None
    peso_liquido   = float(st.session_state.get("_of_pl",  0.0) or 0.0)
    peso_bruto     = float(st.session_state.get("_of_pb",  0.0) or 0.0)
    liga           = (st.session_state.get("_of_liga")    or "").strip() or None
    norma          = (st.session_state.get("_of_norma")   or "").strip() or None
