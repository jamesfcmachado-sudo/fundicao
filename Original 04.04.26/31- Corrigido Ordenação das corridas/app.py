...
def pagina_relatorios() -> None:
    ...
    with tab2:
        if not corridas:
            ...
            # Ordena: 1º pelo ANO de fusão (ignora dia e mês), 2º pela chave alfanumérica
            # Qualquer texto após o padrão 000X0 é ignorado (ex: "001A0 extra" → "001A0")
            if "Corrida" in df.columns and "Data fusão" in df.columns:
                ...
                df = df.sort_values(
                    by=["_sort_ano", "_sort_corrida"],
                    ascending=[True, True],
                    na_position="last",
                ).drop(columns=["_sort_ano", "_sort_corrida"]).reset_index(drop=True)
