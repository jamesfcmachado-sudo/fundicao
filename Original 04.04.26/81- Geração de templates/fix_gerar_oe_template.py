...
if "template_oe_base64" in src:
    print("Ja aplicado!")
    exit(0)

OLD = '''            # Gerar PDF da OE selecionada
            st.divider()
            if st.button("📄 Gerar PDF desta OE", key="btn_pdf_cons"):'''

NEW = '''            # Gerar PDF da OE selecionada
            st.divider()
...
if OLD in src:
    ...
    print("OK: Botao gerar OE com template adicionado.")
else:
...
try:
    ...
    print("SINTAXE OK! Rode: git add . && git commit -m 'Gerar OE com template Excel' && git push")
