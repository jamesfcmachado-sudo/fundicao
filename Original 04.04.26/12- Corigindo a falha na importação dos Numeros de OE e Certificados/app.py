...
def _importar_ofs(arquivo) -> None:
    ...
    for _, row in df.iterrows():
        try:
            ...
            # ── OE: lê numero_oe da linha; também suporta colunas oe1, oe2... ──
            _DASH_IMPORT = frozenset(["-", "—", "–", "nan", "none", "n/a", ""])
            def _clean(col):
                v = _val(col)
                if v is None: return None
                s = str(v).strip()
                return None if s.lower() in _DASH_IMPORT else s

            # Coleta todos os números de OE da linha
            # Suporta coluna única "numero_oe" ou múltiplas "oe1", "oe2", ...
            oe_nums = []
            oe_principal = _clean("numero_oe")
            if oe_principal:
                oe_nums.append(oe_principal)
            for _sfx in range(1, 11):  # oe1 até oe10
                _v = _clean(f"oe{_sfx}") or _clean(f"numero_oe{_sfx}")
                if _v and _v not in oe_nums:
                    oe_nums.append(_v)

            # Coleta todos os números de certificado
            cert_nums = []
            cert_principal = _clean("numero_certificado")
            if cert_principal:
                cert_nums.append(cert_principal)
            for _sfx in range(1, 11):  # cert1 até cert10
                _v = _clean(f"cert{_sfx}") or _clean(f"numero_certificado{_sfx}")
                if _v and _v not in cert_nums:
                    cert_nums.append(_v)
