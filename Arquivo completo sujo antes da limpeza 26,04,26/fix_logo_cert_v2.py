...
OLD = '''            if st.button("💾 Salvar orientação Certificado", key="btn_save_orient_cert"):
                set_config("template_cert_orientacao", _orient_cert)
                st.success(f"✅ Orientação Certificado: {_orient_cert}")

            st.divider()'''

NEW = '''            if st.button("💾 Salvar orientação Certificado", key="btn_save_orient_cert"):
                set_config("template_cert_orientacao", _orient_cert)
                st.success(f"✅ Orientação Certificado: {_orient_cert}")
            ...
            if _up_logo_cert:
                ...
                _b64lc_new = _b64ulc.b64encode(_up_logo_cert.read()).decode()
                set_config("logo_certificado_base64", _b64lc_new)
...
if OLD in src:
    ...
    print("OK: Logo certificado adicionado.")
else:
